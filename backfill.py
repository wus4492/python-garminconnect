...
END_DATE = "2026-07-13"   # 不要跟每日同步重複到，抓到前天即可
# ============================================
...
def collect_day(day):
    ...
    stats = safe(lambda: client.get_stats(day), {}) or {}
    sleep_data = safe(lambda: client.get_sleep_data(day), {}) or {}
    ...
    hrv_data = safe(lambda: client.get_hrv_data(day), {}) or {}
    hrv = (hrv_data.get("hrvSummary") or {}).get("lastNightAvg", "")

    vo2max = ""
    metrics = safe(lambda: client.get_max_metrics(day), []) or []
    if metrics:
    ...
    readiness_score = ""
    readiness_level = ""
    readiness = safe(lambda: client.get_training_readiness(day), []) or []
    if readiness:
    ...
    activities = safe(lambda: client.get_activities_by_date(day, day), []) or []
...
while day <= end:
    ...
    rows = collect_day(day_str)
    if rows:
        sheet.append_rows(rows)
        total += len(rows)
    print(f"{day_str} 完成（{len(rows)} 列）")
    time.sleep(2)  # 放慢速度，避免被 Garmin 限流
    day += datetime.timedelta(days=1)

print(f"回補完成，共寫入 {total} 列")
